...
export class ProtocolItResolver extends SeekJobResolver<'protocol-it'> {
  async resolveMany(seekJobRequest: Request): Promise<JobOfferRaw<'protocol-it'>[]> {
  ...
  async resolveOne(seekJobRequest: Request): Promise<DetailedJobOfferRaw<'protocol-it'>> {
